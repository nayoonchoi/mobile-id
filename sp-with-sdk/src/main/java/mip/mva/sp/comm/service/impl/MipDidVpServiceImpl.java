...
public class MipDidVpServiceImpl implements MipDidVpService, InitializingBean {
	...
	public Boolean reVerifyVP(String svcCode, VP vp) throws SpException {
		...
		if (configBean.getIssuerCheckVc()) {
			...
			vcVerifyProfileParam.setCheckVC(true);
	...
	public String getCI(VP vp) throws SpException {
		...
		try {
			...
			try {
				...
				throw new SpException(MipErrorEnum.SP_UNEXPECTED_MSG_FORMAT, null, "data");
			}
			...
			if (ObjectUtils.isEmpty(proof) && ObjectUtils.isEmpty(proofs)) {
				throw new SpException(MipErrorEnum.SP_UNEXPECTED_MSG_FORMAT, null, "proof");
			}
			...
			if (!ObjectUtils.isEmpty(vpNonce) && vpNonce.length() > ciLength) {
				String ciHex = vpNonce.substring(vpNonce.length() - ciLength);
